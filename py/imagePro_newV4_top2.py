...
for run in range(2,17):
    ...
    if valid_cameras:
        ...
        pred_csv="gap_flush_prediction_top2.csv"
        print(df_pixels_new)

        df_pred_new=pred.pred_gap_top2(df_pixels_new,pred_path,run_no,pred_csv)
        print("--------------------------Run={} Gap&Fush Prediction ------------------------".format(run_no))
        print(df_pred_new)
        print("\n")
        df_pred_new.to_csv(pred_path+"pred_gap_flush_run"+run_no+"top2.csv")
