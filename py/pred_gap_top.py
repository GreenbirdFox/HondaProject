...
def pred_gap_top(df_pixels_new,pred_path,run_no,out_filename):
    ...
    if df_pred.shape[0]>0 and (int(run_no) in df_pred["run_no"].unique()):
        ...
        df_pred=df_pred[df_pred["run_no"]!=int(run_no)].copy(deep=True)


    df_pred_new=pd.DataFrame()

    for measure_location in ["A","B"]: #["A","B","C","D","E","F"]:
        ...
        ###load model
        m1=pickle.load(open("./model/Top_Gap_Location_{}.pickle".format(measure_location),"rb"))
...
def pred_gap_top2(df_pixels_new,pred_path,run_no,out_filename):
    cameras=["Left","Right","Middle Left","Middle"]
    ## Check if directory exist, create if not
    if os.path.exists(pred_path)==False:
        os.mkdir(pred_path)
    ...
    ## Load or create file
    gap_flush_pred_path=pred_path+out_filename
    if os.path.exists(gap_flush_pred_path) ==True:
        df_pred=pd.read_csv(gap_flush_pred_path)
    else:
        df_pred=pd.DataFrame()
    ...
    if df_pred.shape[0]>0 and (int(run_no) in df_pred["run_no"].unique()):
        ...
        if all( i in existing_loc for i in ["A","B"]):
            print("Warning: The prediction result of Top2 is avaliable for run= {}, it will be replaced.".format(run_no))
            df_pred=df_pred[~((df_pred["run_no"]==int(run_no))&(df_pred["loc"].isin(["A","B"])))].copy(deep=True)
    df_pred_new=pd.DataFrame()
    ...
    for measure_location in ["A","B"]: #["A","B","C","D","E","F"]:
        ...
        ###load model
        m1=pickle.load(open("./model/Top2_Gap_Location_{}.pickle".format(measure_location),"rb"))
        # m2=pickle.load(open("./model/Flush_Location_{}.pickle".format(measure_location),"rb"))
        ...
        ## Save prediction
        df_pred_new_loc=pd.DataFrame({"run_no":int(run_no),"loc":measure_location,"Left":np.round(x["Left"],3)
            ,"Middle":np.round(x["Middle"],3),"Middle Left":np.round(x["Middle Left"],3),
            "Right":np.round(x["Right"],3),"Gap_pred(mm)":np.round(gap,3)})
        df_pred_new=pd.concat([df_pred_new,df_pred_new_loc]).sort_values(by=["run_no","loc"]).drop_duplicates()
