...
def plot_result_top(img,bin_edg,df_pix,df_pred,ROI_location):
    ''' Default img= imgM
    ...
    if df_pix.shape[0]>0:
        ...
        for loc in list(df_pix["loc"].unique()):
            ...
            ax1.plot(df_pix.loc[df_pix["loc"]==loc,"x_2"], df_pix.loc[df_pix["loc"]==loc,"y_2"], '+')
        ...
        for loc in list(df_pix["loc"].unique()):
            ...
            ax2.plot(np.max(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start,0), df_pix.loc[df_pix["loc"]==loc,"y_2"]-ROI_y_start, '+')
            pixel_count=round(df_pix.loc[df_pix["loc"]==loc,"pix_dist"],5)
            ax2.text(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start+70,df_pix.loc[df_pix["loc"]==loc,"y_1"]-ROI_y_start,
        ...
        for loc in list(df_pred["loc"].unique()):
            ...
            ax3.plot(np.max(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start,0), df_pix.loc[df_pix["loc"]==loc,"y_2"]-ROI_y_start, '+')
            ...
            if loc=="B":
                ax3.text(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start+70,df_pix.loc[df_pix["loc"]==loc,"y_2"]-ROI_y_start,
                     "V_Difference="+str(gap)+"mm",bbox=dict(facecolor='red', alpha=0.4))
            else:
                ax3.text(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start+70,df_pix.loc[df_pix["loc"]==loc,"y_2"]-ROI_y_start,
                     "Gap="+str(gap)+"mm",bbox=dict(facecolor='red', alpha=0.4))
            # ax3.text(df_pix.loc[df_pix["loc"]==loc,"x_2"]-ROI_x_start+20,df_pix.loc[df_pix["loc"]==loc,"y_1"]-ROI_y_start+100,
