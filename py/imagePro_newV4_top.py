...
import py.pred_gap_top as pred
import py.plot_result_top as pr
...
data_dir='221007-SEA-HON-Pictures/221007-TopCamera_pictures/'
        ...
        while (sum(np.diff(np.array(locys)))<1 or len(locys)<2) and abs(shift)<100 :
            ...
            for i in mpt_ys:
                ...
                # disgard second line the candidate measurepoint _x based on the brightness value. if the 35 neighbors not in bright image.
                    ...
                    if len(locys)>0 and i >locys[-1]+34:#30
                        locys.append(i)
                else:
                    pass
...
for run in range(2,17):
    ...
    if valid_cameras:
        ...
        for camera in valid_cameras:
            ...
            ROI_x_start1,ROI_x_end1,ROI_y_start1,ROI_y_end1=ROI_area(pt1,temp_w1,temp_h1,shift_x,shift_y,"A")
            ROI_x_start2,ROI_x_end2,ROI_y_start2,ROI_y_end2=ROI_area(pt2,temp_w2,temp_h2,shift_x,shift_y,"B")
            ...
            ROI_x_start1,ROI_x_end1,ROI_y_start1,ROI_y_end1=ROI_x_start1,ROI_x_end1,ROI_y_start1,pt11[1]
            ...
            plt.savefig(fig_path+camera+"/"+"Run"+run_no+"-"+camera+"-Top-ROI-Area.png")
            plt.close("all")
            ...
            plt.savefig(fig_path+camera+"/"+"Run"+run_no+"-"+camera+"-TopStep-ROI-Area.png")
            plt.close("all")
            ...
            pixel_out.to_csv(csv_path+camera+"/"+"Run"+run_no+"-"+camera+"Top-pixels.csv")
            print("Saved to "+csv_path+camera+"/"+"Run"+run_no+"-"+camera+"Top-pixels.csv")
        ...
        df_pixels_all.to_csv(csv_path+"allcameras_pixels_count_top_fromImageProcess.csv",index=False)
        ...
        pred_csv="gap_flush_prediction_top.csv"
        print(df_pixels_new)


        df_pred_new=pred.pred_gap_top(df_pixels_new,pred_path,run_no,pred_csv)
        print("--------------------------Run={} Gap&Fush Prediction ------------------------".format(run_no))
        print(df_pred_new)
        print("\n")
        df_pred_new.to_csv(pred_path+"pred_gap_flush_run"+run_no+"top.csv")
        ...
        for camera in valid_cameras:
            ...
            fig1=pr.plot_result_top(images[camera],bin_edg_images[camera],df_pixes_camera,df_pred_new,ROI_loc[camera])
            ...
            if os.path.exists(fig_path+camera+"/") ==False:
                os.mkdir(fig_path+camera+"/")
                print("Directory {} created".format(fig_path+camera+"/"))
            fig1.savefig(fig_path+camera+"/"+"Run"+run_no+"-"+camera+"-ROI-Top1-MeasurePoint.png")
        ...
        if df_pred.shape[0]>0:
            for loc in df_pred["loc"].unique():
                ...
                if df_pred_loc.shape[0]>=3:
                    ...
                    chart1.figure.savefig(chart_path+loc+"-Top-hotelling-chart.png")
                    ...
                    # chart2.suptitle(loc)  
                    chart3.savefig(chart_path+loc+"-Top-EWMA-chart.png")
